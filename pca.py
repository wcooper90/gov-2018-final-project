...
top_5_percentile = np.percentile(PC1_scores, 95)
bottom_5_percentile = np.percentile(PC1_scores, 5)
counter = 0
for i, score in enumerate(PC1_scores):
    if score > top_5_percentile:
        print(i, full_df[i:i + 1]['county_name_x'].iloc[0])
        counter += 1
    if counter > 15:
        break
print("*"*80)
...
top_5_percentile = np.percentile(PC2_scores, 95)
bottom_5_percentile = np.percentile(PC2_scores, 5)
print("*"*80)
